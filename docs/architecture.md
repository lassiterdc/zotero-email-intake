...
**Implemented**: `0.1.1`, tagged `v0.1.1`. Every decision recorded below is embodied in the artifact published at that tag; a reader comparing this document against a later working tree should date the difference from there rather than assume the document is stale.
...
**One environmental prerequisite is NOT satisfiable by this plugin's code, and it must be stated because "let the ecosystem react" otherwise reads as requiring nothing of the user.** ZotMoov filters automove candidates through `allowed_fileext`, whose shipped default is `["pdf","epub","docx","djvu"]`. An `.eml` is filtered out before any of the user's other ZotMoov settings are consulted, so with a stock ZotMoov the promoted attachment is renamed correctly and then simply stays in Zotero storage. Measured 2026-08-16 on Zotero 9.0.6: with the stock allowlist the file remained at `~/Zotero/storage/{key}`; with `eml` appended it filed to `{dst_dir}/E-mail/Public/` on the next drop. **The user must add BOTH `eml` and `msg` to `extensions.zotmoov.allowed_fileext`**, and no amount of correctness on this side substitutes for it. The `msg` half is owed from Phase 3b, which adopted `.msg` as a promotable format; a prerequisite naming only `eml` after that point describes the plugin as it was rather than as it ships, and the failure it produces is identical — the container promotes and renames correctly and then silently stays in Zotero storage. Confirmed 2026-08-19 on the same machine: with `msg` appended, a header-less `.msg` promoted from the MAPI fallback and filed to `{dst_dir}/E-mail/Whitfield/` as `Whitfield_Quarterly planning notes.msg`. The comparison is case-insensitive and the entries carry no leading dot — ZotMoov lowercases both the allowlist and the file's extension before testing membership — so `eml` and `msg` are the exact tokens, not `.eml`/`.msg` and not uppercase variants.
...
| ZotMoov extension allowlist | `allowed_fileext` = `["pdf","epub","docx","djvu"]` — **both `eml` and `msg` absent by default**; each must be added by the user, and the `msg` requirement dates from Phase 3b's adoption of that format | `zotmoov/prefs.js` |
...
**The bound is PER-FORMAT from Phase 3b, when `.msg` adoption lands.** `.eml` keeps `262144`: a 256 KB prefix contains any RFC 5322 header region by construction, so raising it would admit more attacker-controlled bytes for no capability. A CFB container cannot be served by a prefix at all — its FAT sectors, directory chain and target stream may sit anywhere in the file, and the reader rejects any sector extending past the buffer — so `.msg` reads under a separate `CONTAINER_READ_CAP` of **33554432 (32 MB)**. That number is grounded in what actually bounds a real `.msg`: a message that arrived over SMTP is limited by the sending server's attachment ceiling, and the common ceilings are Gmail 25 MB, Outlook.com 20 MB and Exchange defaults in the 25-35 MB range, so 32 MB clears the usual limit with headroom for MSG and RTF overhead. Above the cap the file is refused by `IOUtils.stat` BEFORE any read, with `E_CONTAINER_TOO_LARGE`.

The residual is stated rather than hidden: **internal organisation mail is not bounded by an internet-facing limit**, and org ceilings can run to 50-150 MB, so some large internal messages will decline. That decline is visible and carries its own code, which is the whole difference from the silent-truncation failure a prefix read would have produced — the CFB reader fails closed, so a truncated container yields a clean null indistinguishable from a genuine parse failure. Choosing a bound that cannot be silently exceeded is what makes the refusal honest.
...
- **The read bound is per-format at the single read site.** `HEADER_READ_CAP` stays at 262144 for `.eml` and a new `CONTAINER_READ_CAP` of 32 MB serves `.msg`, because a CFB directory walk needs the whole file while a 256 KB prefix contains any RFC 5322 header region by construction. The oversize check runs on `IOUtils.stat` BEFORE the read, so refusing a 200 MB container never allocates 200 MB, and it raises its own taxonomy code rather than reusing `E_NO_HEADER_TERMINATOR` — that code asserts "the header block has no blank-line terminator", a claim that cannot be made about a file which was never opened.
